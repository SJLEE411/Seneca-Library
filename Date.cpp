...
namespace sdds {
   ...
   bool Date::validate() {
      errCode(NO_ERROR);
      if (m_year < MIN_YEAR || m_year > m_CUR_YEAR + 1) { // +1 should not be here to not accept 2024 // update: was fixed on matrix output
         errCode(YEAR_ERROR);
      }
   ...
   //
    Date::Date(const char* date) {
      ...
      int year, mon, day;
      if (std::sscanf(date, "%d/%d/%d", &year, &mon, &day) == 3) {
      m_year = year;
      m_mon = mon;
      m_day = day;
      validate();
